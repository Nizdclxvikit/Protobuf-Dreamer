...
class Dreamer:
    def __init__(self, tile_size=512, print_model=True, verbose=True, initialize=True):
        if initialize:
    ...
    #last_grad = None
    #last_channel = None
    def render(self, img, layer='mixed4d_3x3_bottleneck_pre_relu', channel=-1, iter_n=10, step=1.5, octave_n=4, octave_scale=1.4):
...
if __name__ == "__main__":
    ...
    dreamer = Dreamer(tile_size=tile_size, print_model=print_model, verbose=verbose)

    output_img = dreamer.render(input_img, layer=layer_name, channel=channel_value, iter_n=iter_value, step=step_size, octave_n=octave_value, octave_scale=octave_scale_value)
